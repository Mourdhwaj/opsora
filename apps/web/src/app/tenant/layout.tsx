...
const navItems = [
  { label: "My Dashboard", href: "/tenant", icon: LayoutDashboard },
  { label: "My Payments", href: "/tenant/payments", icon: CreditCard },
  { label: "My Complaints", href: "/tenant/complaints", icon: AlertTriangle },
  { label: "Food & Meals", href: "/tenant/food", icon: UtensilsCrossed },
  { label: "My Profile", href: "/tenant/profile", icon: User },
];
      ...
      <aside className={cn(
        ...
        <div className="h-16 flex items-center gap-3 px-5 border-b border-border flex-shrink-0">
          ...
          <div>
            ...
            <span className="block text-[10px] text-ink-muted -mt-0.5">Resident Portal</span>
          </div>
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                  className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150",
                    isActive ? "bg-accent-light text-accent" : "text-ink-secondary hover:bg-canvas hover:text-ink"
                  )}>
                  <item.icon className={cn("w-[18px] h-[18px] flex-shrink-0", isActive && "text-accent")} />
                  {item.label}
                </Link>
              );
            })}
        ...
        <div className="p-3 border-t border-border flex-shrink-0">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-canvas">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-ink truncate">{user?.fullName || "Resident"}</p>
              <p className="text-xs text-ink-muted capitalize">{user?.role || "resident"}</p>
            </div>
            <button onClick={logout} className="p-1.5 rounded-md text-ink-muted hover:text-danger hover:bg-danger-light transition-colors" title="Sign out">
