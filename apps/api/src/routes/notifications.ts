...
export async function notificationRoutes(app: FastifyInstance) {
  app.get('/notifications', { preHandler: [authenticate] }, async (request, reply) => {
    ...
    const total = db.select({ count: sql<number>`count(*)` }).from(notifications).where(and(...conditions)).get()?.count ?? 0;
    const unreadCount = db.select({ count: sql<number>`count(*)` }).from(notifications).where(and(eq(notifications.tenantId, tenantId), eq(notifications.userId, userId), eq(notifications.isRead, false))).get()?.count ?? 0;
  ...
  app.get('/activity-logs', { preHandler: [authenticate] }, async (request, reply) => {
    ...
    const total = db.select({ count: sql<number>`count(*)` }).from(activityLogs).where(and(...conditions)).get()?.count ?? 0;
